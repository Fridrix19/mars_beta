...
export default defineEventHandler(async (e) => {
  ...
  const role = b?.role && ['owner', 'operator', 'kyc'].includes(b.role) ? b.role : t.role
  ...
  await q(`update admins set role = $2, active = $3, password_hash = $4, must_change = $5 where id = $1`, [t.id, role, active, hash, mustChange])
  if (!active || b?.password) await q(`delete from admin_sessions where admin_id = $1`, [t.id])
  await audit(e, a, 'admin.update', t.id, { login: t.login, role, active, password_reset: !!b?.password })
  return { ok: true }
})
