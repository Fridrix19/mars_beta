...
export default defineEventHandler(async (e) => {
  ...
  const role = ['owner', 'operator', 'kyc'].includes(b?.role) ? b.role : 'operator'
  const pw = String(b?.password ?? '')
  if (pw.length < 10) fail(422, 'weak_password', 'Временный пароль — от 10 символов.')
  const r = await one(`insert into admins (login, password_hash, name, role, must_change) values ($1, $2, $3, $4, true) returning id, login, name, role`,
    [login, await hashPassword(pw), name.slice(0, 80), role]).catch((err: any) => { if (err?.code === '23505') fail(409, 'login_taken', 'Такой логин уже есть.'); throw err })
